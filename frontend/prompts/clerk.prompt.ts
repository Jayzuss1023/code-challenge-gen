export const CLERK_AUTH_PROMPT = `
...
This setup is linked to the Clerk application \`app_3IJrES9QItyViEJRcUBBG5d7cO4\`. Always
pass \`--app app_3IJrES9QItyViEJRcUBBG5d7cO4\` to \`clerk init\` so the project links to
the right Clerk app.

If this is an existing project, run:

\`\`\`bash
clerk init --app app_3IJrES9QItyViEJRcUBBG5d7cO4
\`\`\`
...
\`\`\`bash
clerk init --framework <framework> --pm <package-manager> --app app_3IJrES9QItyViEJRcUBBG5d7cO4
\`\`\`
...
Docs: https://clerk.com/docs/cli https://clerk.com
