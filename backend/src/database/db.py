...
# Get a User's challenge question
...
# Initiate quota for user for first time sign up.
...
def reset_quota_if_needed(db: Session, quota: models.ChallengeQuota):
    now = datetime.now()
    last_reset = _as_datetime(quota.last_reset_date)
...
# Challenge created. It's values come from the generation made by AI
...
# Get all challenges that User has generated
def get_user_challenges(db: Session, user_id):
